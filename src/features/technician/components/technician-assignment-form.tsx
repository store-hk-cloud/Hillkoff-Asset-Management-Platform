    ...
    try {
      ...
      setError(
        reason instanceof Error
          ? reason.message
          : thaiPrimary(locale, "มอบหมายงานไม่สำเร็จ", "Assignment failed."),
      );
    } finally {
      setBusy(false);
    }
  ...
  return (
    <form className="space-y-3" onSubmit={submit}>
      <Label htmlFor={`technician-${type}-${workId}`}>
        {thaiPrimary(locale, "เลือกช่างผู้รับผิดชอบ", "Select technician")}
      </Label>
      <TechnicianSelect
      ...
      <Button disabled={busy} type="submit">
        {thaiPrimary(locale, "มอบหมายงาน", "Assign work")}
      </Button>
    </form>
  );
}
