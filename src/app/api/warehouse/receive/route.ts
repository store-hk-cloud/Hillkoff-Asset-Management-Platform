...
import { receiveAssetSchema } from "@/features/warehouse/schemas/movement.schema";
...
export async function POST(request: Request) {
  ...
  try {
    const input = receiveAssetSchema.parse(await request.json());
    const movement = await warehouseService.receive(
      input,
    ...
    return NextResponse.json(
      {
        success: true,
        data: { id: movement.id, movementNumber: movement.movementNumber },
      },
      { status: 201 },
    );
  } catch (error) {
