...
import { rejectTransferSchema } from "@/features/warehouse/schemas/movement.schema";
...
export async function POST(
  ...
  try {
    const input = rejectTransferSchema.parse(await request.json());
    const { transferId } = await context.params;
    const transfer = await service.rejectTransfer(
      transferId,
      input,
